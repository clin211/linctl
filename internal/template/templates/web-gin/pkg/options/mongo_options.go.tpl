...
// MongoOptions 定义连接 MongoDB 服务器所需的配置项。
...
// NewMongoOptions 构造一个带默认值的 *MongoOptions。
...
// AddFlags 把 MongoOptions 上的字段注册为命令行 flag。
func (o *MongoOptions) AddFlags(fs *pflag.FlagSet, fullPrefix string) {
	o.TLSOptions.AddFlags(fs, fullPrefix+".tls")

	fs.DurationVar(&o.Timeout, fullPrefix+".timeout", o.Timeout, "拨号建立连接的最大等待时长")
	fs.StringVar(&o.URL, fullPrefix+".url", o.URL, "MongoDB 服务地址")
	fs.StringVar(&o.Database, fullPrefix+".database", o.Database, "MongoDB 数据库名")
	fs.StringVar(&o.Collection, fullPrefix+".collection", o.Collection, "MongoDB 集合名")
	fs.StringVar(&o.Username, fullPrefix+".username", o.Username, "MongoDB 用户名（可选）")
	fs.StringVar(&o.Password, fullPrefix+".password", o.Password, "MongoDB 密码（可选）")
}

// NewClient 根据当前配置创建一个 MongoDB 客户端。
